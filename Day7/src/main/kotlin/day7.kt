...
fun executeProgram(inputValue: Queue<Int>, programMemory: MutableList<Int>): Int {
    var index = 0
    var outputValue = 0
    ...
    while (true) {
        ...
        when (opcode) {
            ...
            INPUT -> {
                programMemory[programMemory[index+1]] = inputValue.remove()
                index += 2
            }
            OUTPUT -> {
                outputValue = getInput(programMemory[index+1], input1Mode, programMemory)
                index += 2
            }
...
fun amplifiersPart1(phases: List<Int>, program: List<Int>): Int {
    ...
    for (phase in phases) {
        signal = executeProgram(LinkedList(listOf(phase, signal)), program.toMutableList())
    }
